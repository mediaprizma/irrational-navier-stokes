...
interface Props {
  irrHistory: PeakData[];
  harmHistory: PeakData[];
  irrMax: number;
  harmMax: number;
  height?: number;
}

export default function PeakComparison({ irrHistory, harmHistory, irrMax, harmMax, height = 200 }: Props) {
  ...
  useEffect(() => {
    ...
    if (irrHistory.length < 2 && harmHistory.length < 2) {
      ...
      ctx.fillText('Start simulation to see peak comparison', W / 2, H / 2);
      ctx.textAlign = 'left';
      return;
    }
    ...
    ctx.fillStyle = 'rgb(0, 220, 255)';
    ctx.fillText(`IRRATIONAL: ${irrMax.toExponential(2)}`, pad.left + 8, pad.top + 14);
    ctx.fillStyle = 'rgb(200, 100, 255)';
    ctx.fillText(`HARMONIC: ${harmMax.toExponential(2)}`, pad.left + 8, pad.top + 28);
    ...
    ctx.fillText(`Peak=${maxPeak.toExponential(1)}`, 2, pad.top + 4);
    ...
    ctx.font = '9px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('Peak Energy (V²)', 0, 0);
    ctx.restore();
